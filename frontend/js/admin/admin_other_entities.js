...
function renderResearchFieldsTable(dataList) {
    ...
    tbody.innerHTML = dataList.map((lv) => {
        ...
        return `
            <tr>
                <td>${lv.id || 'Chưa rõ'}</td>
                <td><strong>${lv.ten_linh_vuc || 'Chưa rõ'}</strong></td>
...
function renderExternalAuthorsTable(dataList) {
    ...
    tbody.innerHTML = dataList.map((tgn) => {
        ...
        return `
        <tr>
            <td>${tgn.id || 'Chưa rõ'}</td>
            <td><strong>${tgn.ho_va_ten || 'Chưa rõ'}</strong>${statusBadge}</td>
        ...
        if (data.status === 'ok') {
            ...
            if (tbody) {
                tbody.innerHTML = data.data.map((bm, i) => `
                    <tr>
                        <td>${bm.id || i + 1}</td>
                        <td><strong>${bm.ten_bo_mon || 'Chưa rõ'}</strong></td>
                        <td style="text-align: center;">
                            <span class="badge" style="background: rgba(0, 123, 255, 0.1); color: #007bff; padding: 3px 8px; border-radius: 4px; font-weight: 600;">
                                ${bm.so_luong_gv || 0}
                            </span>
                        </td>
                        <td>
                            <div style="display: flex; gap: 5px;">
                                <button class="btn btn-sm" style="background:#f39c12;color:#fff;border-color:#f39c12;" title="Xem chi tiết" onclick="viewDepartmentDetail('${bm.id}')"><i class="fas fa-eye"></i></button>
