...
@admin_research_fields_bp.route("/linh-vuc/<int:id>", methods=["PUT"])
def update_linh_vuc(id):
    data = request.json
    conn = get_neo4j_connection()
    try:
        conn.write("""
            MATCH (lv:LinhVucNghienCuu) WHERE id(lv) = $id
...
@admin_research_fields_bp.route("/linh-vuc/<int:id>", methods=["DELETE"])
