...
# ==============================================================================
@admin_relations_bp.route("/cong-trinh/<int:ct_id>/giang-vien", methods=["GET"])
def get_tac_gia_cong_trinh(ct_id):
    ...
    results = conn.query("""
        ...
        WHERE id(ct) = $id
        RETURN id(gv) AS gv_id, gv.ho_va_ten AS ten
    """, {"id": ct_id})
    data = [{"id": r["gv_id"], "ten": r["ten"]} for r in results]
    return jsonify({"status": "ok", "data": data})


@admin_relations_bp.route("/cong-trinh/<int:ct_id>/giang-vien", methods=["PUT"])
    ...
    try:
        ...
        if gv_ids:
            ...
            conn.query("""
                ...
                WHERE id(gv) = gv_id AND id(ct) = $id
...
# ==============================================================================
@admin_relations_bp.route("/de-tai/<int:dt_id>/giang-vien", methods=["GET"])
def get_thanh_vien_de_tai(dt_id):
    ...
    results = conn.query("""
        ...
        WHERE id(dt) = $id
        RETURN id(gv) AS gv_id, gv.ho_va_ten AS ten, type(r) AS vai_tro
    """, {"id": dt_id})
    data = [{"id": r["gv_id"], "ten": r["ten"], "vai_tro": r["vai_tro"]} for r in results]
    return jsonify({"status": "ok", "data": data})


@admin_relations_bp.route("/de-tai/<int:dt_id>/giang-vien", methods=["PUT"])
    ...
    try:
        ...
        if chu_nhiem_ids:
            conn.query("""
                ...
                WHERE id(gv) = gv_id AND id(dt) = $id
